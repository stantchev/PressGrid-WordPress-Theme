...
$side_posts = array_slice( $all_posts, 0, 3 );
?>
<section class="pg-section pg-hero" aria-label="<?php esc_attr_e( 'Featured Stories', 'pressgrid' ); ?>">
	<div class="pg-hero-grid">
		...
		<div class="pg-hero-main">
			...
			<?php else : ?>
				<div style="width:100%;min-height:320px;background:#222;display:flex;align-items:center;justify-content:center;">
			...
			<div class="pg-hero-caption">
				...
				if ( $exc ) : ?>
					<p><?php echo esc_html( wp_trim_words( $exc, 18 ) ); ?></p>
				<?php endif; ?>
			</div>
		</div>
		<?php endif; ?>

		<!-- ── RIGHT: Side stack (pure flexbox, NO inner grid) ── -->
		<?php if ( ! empty( $side_posts ) ) : ?>
		<div class="pg-hero-sidebar">
			<?php foreach ( $side_posts as $side ) :
				$sthumb = get_the_post_thumbnail_url( $side->ID, 'pressgrid-thumb' );
			?>
			<article class="pg-hero-side-item">
				...
				<div class="pg-hero-side-body">
					<?php echo wp_kses_post( pressgrid_get_category_label( $side->ID ) ); ?>
					...
					if ( $sex ) : ?>
						<p><?php echo esc_html( wp_trim_words( $sex, 14 ) ); ?></p>
					<?php endif; ?>
				</div>
			</article>
			<?php endforeach; ?>
